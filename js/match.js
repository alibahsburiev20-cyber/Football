...
const Match = (() => {
  ...
  function buildTeams(config) {
    ...
    // HOME (пользователь играет за нападающего home, остальные — AI той же команды)
    const homeFormationX = [b.left + 60, b.cx - 260, b.cx - 130, b.cx - 60];
    const homeFormationY = [b.cy, b.cy - 150, b.cy + 150, b.cy];

    players.push(Entities.createPlayer({ x: homeFormationX[0], y: homeFormationY[0], team: 'home', role: 'gk' }));
    players.push(Entities.createPlayer({ x: homeFormationX[1], y: homeFormationY[1], team: 'home', role: 'defender' }));
    players.push(Entities.createPlayer({ x: homeFormationX[2], y: homeFormationY[2], team: 'home', role: 'defender' }));
    players.push(Entities.createPlayer({ x: homeFormationX[3], y: homeFormationY[3], team: 'home', role: 'striker', isUser: true }));

    const awayFormationX = [b.right - 60, b.cx + 260, b.cx + 130, b.cx + 60];
    const awayFormationY = [b.cy, b.cy - 150, b.cy + 150, b.cy];

    players.push(Entities.createPlayer({ x: awayFormationX[0], y: awayFormationY[0], team: 'away', role: 'gk' }));
    players.push(Entities.createPlayer({ x: awayFormationX[1], y: awayFormationY[1], team: 'away', role: 'defender' }));
    players.push(Entities.createPlayer({ x: awayFormationX[2], y: awayFormationY[2], team: 'away', role: 'defender' }));
    players.push(Entities.createPlayer({ x: awayFormationX[3], y: awayFormationY[3], team: 'away', role: 'striker' }));
  ...
  function resetPlayerPositions() {
    const b = Field.getPlayBounds();
    const homeX = [b.left + 60, b.cx - 260, b.cx - 130, b.cx - 60];
    const homeY = [b.cy, b.cy - 150, b.cy + 150, b.cy];
    const awayX = [b.right - 60, b.cx + 260, b.cx + 130, b.cx + 60];
    const awayY = [b.cy, b.cy - 150, b.cy + 150, b.cy];

    const homePlayers = state.players.filter(p => p.team === 'home');
    const awayPlayers = state.players.filter(p => p.team === 'away');
    homePlayers.forEach((p, i) => { p.x = homeX[i]; p.y = homeY[i]; p.vx = 0; p.vy = 0; p.ai.homeX = homeX[i]; p.ai.homeY = homeY[i]; });
    awayPlayers.forEach((p, i) => { p.x = awayX[i]; p.y = awayY[i]; p.vx = 0; p.vy = 0; p.ai.homeX = awayX[i]; p.ai.homeY = awayY[i]; });
  }
